...
\begin{algorithm}[H]
    \KwIn{Tableau $source$ de taille $L \times C$,
    nombre de couleurs réduites $r$, nombre $f$ de filtrages à effectuer.}
    \KwResult{Tableau $destination$ passé dans $source$ par référence\\
...
\strut\vspace*{-\baselineskip}\newline
\begin{algorithm}[H]
    \KwIn{Coordonnées $x$, $y$ du pixel dont on calcule la valeur, tableau $source$ (input du filtrage)}
    \KwResult{Valeur à donner au pixel\\
    \textbf{Remarque:} \texttt{maxSameVals} et \texttt{filterColor} sont des constantes du problème, valant ici respectivement 6 et 0.\\\hrulefill}
    $count$ est une liste de longueur fixe $r + 1$.\\
    $current \gets 0$
      	  ...
      	  \Pour{$j$ \upshape{de} $-1$ \upshape{à} $1$}
      	    {
    	       \Si{$i \neq 0$ \upshape{ou} $j \neq 0$}
    	       {
    	       		...
    	       		\Pour{$c$ \upshape{de} 0 \upshape{à} $r$}
    	       		{
    	       			...
    	       			{
    	       			 $count$[$c$]$= count$[$c$]$ + 1$\\
    	       			 \Si{$count[c]\geq$ \upshape{\texttt{maxSameVals}}}
    	       			 {
    	       			 \textbf{Sortir} $c$\\
    	       			 \textbf{Fin}
    	       			 } 
       ...
       \textbf{Sortir} \upshape\texttt{filterColor}\\
       \textbf{Fin}
\caption{\texttt{getPixelValues(x, y, r,} $source$\texttt{)} : Valeur d'un pixel en fonction de ses voisins}
\end{algorithm}
\end{minipage}
...
legend style={font=\tiny,at={(0.1,0.764)}, anchor=south west, legend cell align=left, align=left, draw=white!11!black}
]
...
{\blfootnote{Sur ce graphique, on voit le temps d'exécution de mon programme \texttt{projet} sur le fichier test \texttt{tree.txt} selon le nombre de filtrage appliqué, en comparaison avec le programme \texttt{demo} ainsi que le double du temps mis par le programme \texttt{demo}.}}
